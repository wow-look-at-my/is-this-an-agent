#!/bin/sh
# is-this-codex.sh -- is this shell running under Codex?
#
# Exit status: 0 when running under Codex, 1 when not. With -q it prints nothing, so it
# reads well in a conditional:
#
#     if is-this-codex.sh -q; then ...; fi
#
...
usage() {
    cat <<EOF
usage: is-this-codex.sh [-q] [-h]

Report whether this shell is running under Codex.
...
for arg in "$@"; do
    case $arg in
        -q|--quiet) quiet=1 ;;
        -h|--help) usage; exit 0 ;;
        *) printf '%s: unknown option: %s\n' "is-this-codex.sh" "$arg" >&2; usage >&2; exit 2 ;;
    esac
done

# Codex exports no "you are under Codex" marker. What it does export is its
...
AGENT_NAME='Codex'
AGENT_ENV_VARS='CODEX_SANDBOX CODEX_SANDBOX_NETWORK_DISABLED'
AGENT_PROCS='codex'
AGENT_PID_VARS=''
