#!/bin/sh
# is-this-gemini.sh -- is this shell running under Gemini CLI?
#
# Exit status: 0 when running under Gemini CLI, 1 when not. With -q it prints nothing, so it
# reads well in a conditional:
#
#     if is-this-gemini.sh -q; then ...; fi
#
...
usage() {
    cat <<EOF
usage: is-this-gemini.sh [-q] [-h]

Report whether this shell is running under Gemini CLI.
...
for arg in "$@"; do
    case $arg in
        -q|--quiet) quiet=1 ;;
        -h|--help) usage; exit 0 ;;
        *) printf '%s: unknown option: %s\n' "is-this-gemini.sh" "$arg" >&2; usage >&2; exit 2 ;;
    esac
done

AGENT_NAME='Gemini CLI'
AGENT_ENV_VARS='GEMINI_CLI'
AGENT_PROCS='gemini'
AGENT_PID_VARS=''
