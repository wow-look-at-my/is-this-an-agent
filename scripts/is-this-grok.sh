#!/bin/sh
# is-this-grok.sh -- is this shell running under grok build?
#
# Exit status: 0 when running under grok build, 1 when not. With -q it prints nothing, so it
# reads well in a conditional:
#
#     if is-this-grok.sh -q; then ...; fi
#
...
usage() {
    cat <<EOF
usage: is-this-grok.sh [-q] [-h]

Report whether this shell is running under grok build.
...
for arg in "$@"; do
    case $arg in
        -q|--quiet) quiet=1 ;;
        -h|--help) usage; exit 0 ;;
        *) printf '%s: unknown option: %s\n' "is-this-grok.sh" "$arg" >&2; usage >&2; exit 2 ;;
    esac
done
