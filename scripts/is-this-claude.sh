#!/bin/sh
# is-this-claude.sh -- is this shell running under Claude?
#
# Exit status: 0 when running under Claude, 1 when not. With -q it prints nothing, so it
# reads well in a conditional:
#
#     if is-this-claude.sh -q; then ...; fi
#
...
usage() {
    cat <<EOF
usage: is-this-claude.sh [-q] [-h]

Report whether this shell is running under Claude.
...
for arg in "$@"; do
    case $arg in
        -q|--quiet) quiet=1 ;;
        -h|--help) usage; exit 0 ;;
        *) printf '%s: unknown option: %s\n' "is-this-claude.sh" "$arg" >&2; usage >&2; exit 2 ;;
    esac
done

AGENT_NAME='Claude'
AGENT_ENV_VARS='CLAUDECODE'
AGENT_PROCS='claude'
AGENT_PID_VARS=''
