...
# THE source of this block. Every scripts/is-this-*.sh embeds it verbatim,
# because those scripts are standalone -- copy one anywhere and run it, with
# nothing to source. They are GENERATED (`go run ./cmd/gen-scripts`) from this
# file and the Go roster, and a test regenerates them to fail on drift: do not
# edit a generated script, edit this file or agent.go.
#
